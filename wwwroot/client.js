...
connection.on("ReceiveAddMessage", function (scores) {
    ...
    element.addEventListener('animationend', () => { element.classList.remove('highlight-add'); });
});
...
connection.on("ReceiveDeleteMessage", function (scores) {
    ...
    element.addEventListener('animationend', () => { element.classList.remove('highlight-delete'); });
});
